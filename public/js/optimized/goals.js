...
class GoalsManager {
  ...
  async loadGoals() {
    try {
      ...
      // Use the direct API or fallback to window.apiClient
      if (window.directGoalsApi) {
        this.goals = await window.directGoalsApi.getActivityGoals(
        ...
        console.error('No API client available for loading goals');
        this.goals = [];
      }
    } catch (error) {
