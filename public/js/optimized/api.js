...
class ApiClient {
  ...
  async request(endpoint, method = 'GET', data = null, options = {}) {
    ...
    const token = this._getToken();
    if (token && this._isTokenExpiring(token)) {
  ...
  async login(username, password) {
    return this.request('/auth/login', 'POST', { username, password }, { useCache: false });
  }
   ...
   * @returns {Promise<Array>} Achievement list
   */
  async getAchievements() {
    ...
    return this.request(`/achievements/${user.id}`);
  }
  
  /**
   * Get all achievement types
   * @returns {Promise<Array>} Achievement type list
   */
  async getAchievementTypes() {
    return this.request('/achievements/types');
  }
  
  /**
   * Check for new achievements
   * @param {number} activityTypeId - Activity Type ID (optional)
   * @returns {Promise<Object>} New achievements
   */
  ...
  async shareAchievement(achievementId, platform) {
    return this.request(`/achievements/share/${achievementId}`, 'POST', {
      platform
    });
  }
}
